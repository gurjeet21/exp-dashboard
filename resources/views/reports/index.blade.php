<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-gray-500">German monthly maintenance reports</p>
                <h1 class="mt-1 text-2xl font-semibold text-gray-950">Wartungsberichte</h1>
            </div>
            <a href="{{ route('reports.create') }}" wire:navigate class="rounded-md bg-gray-950 px-4 py-2 text-sm font-semibold text-white">New Report</a>
        </div>
    </x-slot>
            ...
            <form method="GET" action="{{ route('reports.index') }}" class="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                <div class="grid gap-4 lg:grid-cols-6">
                    <div class="lg:col-span-2">
                        ...
                        <input id="search" name="search" value="{{ $filters['search'] ?? '' }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm" placeholder="Client or project">
                    </div>
                    <div>
                    ...
                    <div>
                        <label for="year" class="text-xs font-semibold uppercase text-gray-500">Year</label>
                    ...
                    <div>
                        <label for="month" class="text-xs font-semibold uppercase text-gray-500">Month</label>
                    ...
                    <div>
                        <label for="project_id" class="text-xs font-semibold uppercase text-gray-500">Project</label>
                ...
                <div class="mt-4 grid gap-4 sm:grid-cols-4">
                    <div>
                        <label for="date_from" class="text-xs font-semibold uppercase text-gray-500">Date from</label>
                    ...
                    <div>
                        <label for="date_to" class="text-xs font-semibold uppercase text-gray-500">Date to</label>
                    ...
                    <div class="flex items-end gap-2">
                        ...
                        <a href="{{ route('reports.index') }}" wire:navigate class="rounded-md border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700">Reset</a>
                    </div>
                </div>
            </form>
            ...
            <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
                <table class="min-w-full divide-y divide-gray-100">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-semibold uppercase text-gray-500">Report</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold uppercase text-gray-500">Client</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold uppercase text-gray-500">Hours</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($reports as $report)
                            <tr>
                                ...
                                <td class="px-6 py-4"><span class="rounded-md bg-gray-100 px-2 py-1 text-xs font-semibold text-gray-700">{{ ucfirst($report->status) }}</span></td>
                            </tr>
                        @empty
                            <tr><td colspan="4" class="px-6 py-10 text-center text-sm text-gray-500">No reports yet.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
