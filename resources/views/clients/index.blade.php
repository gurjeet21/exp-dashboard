<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-gray-500">Client Management</p>
                <h1 class="mt-1 text-2xl font-semibold text-gray-950">Clients</h1>
            </div>
            <a href="{{ route('clients.create') }}" wire:navigate class="rounded-md bg-gray-950 px-4 py-2 text-sm font-semibold text-white">New Client</a>
        </div>
    </x-slot>
            ...
            <form method="GET" action="{{ route('clients.index') }}" class="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                <div class="grid gap-4 lg:grid-cols-6">
                    <div class="lg:col-span-2">
                        ...
                        <input id="search" name="search" value="{{ $filters['search'] ?? '' }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm" placeholder="Company, contact, email, code">
                    </div>
                    <div>
                    ...
                    <div>
                        <label for="customer_value" class="text-xs font-semibold uppercase text-gray-500">Value</label>
                    ...
                    <div>
                        <label for="industry" class="text-xs font-semibold uppercase text-gray-500">Industry</label>
                ...
                <div class="mt-4 flex gap-2">
                    ...
                    <a href="{{ route('clients.index') }}" wire:navigate class="rounded-md border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700">Reset</a>
                </div>
            </form>
            ...
            <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
                <table class="min-w-full divide-y divide-gray-100">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-semibold uppercase text-gray-500">Company</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold uppercase text-gray-500">Contact</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold uppercase text-gray-500">Projects</th>
                        ...
                        @forelse ($clients as $client)
                            <tr>
                                <td class="px-6 py-4">
                                    ...
                                    <p class="text-sm text-gray-500">{{ $client->email ?: 'No email yet' }}</p>
                                </td>
                                ...
                                <td class="px-6 py-4"><span class="rounded-md bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700">{{ ucfirst($client->status) }}</span></td>
                            </tr>
                        @empty
                            <tr><td colspan="4" class="px-6 py-10 text-center text-sm text-gray-500">No clients yet. Create your first client to start a project.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
